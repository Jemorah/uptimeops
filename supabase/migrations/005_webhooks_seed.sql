...
-- ═══════════════════════════════════════════
-- DATABASE WEBHOOKS
-- Trigger edge functions on table changes
-- ═══════════════════════════════════════════
...
BEGIN
  PERFORM
    net.http_post(
      url := current_setting('app.supabase_url', true) || '/functions/v1/ai-orchestrator',
...
BEGIN
  IF NEW.status = 'awaiting_approval' AND OLD.status IS DISTINCT FROM 'awaiting_approval' THEN
    PERFORM
      net.http_post(
        url := current_setting('app.supabase_url', true) || '/functions/v1/communication-sender',
        headers := jsonb_build_object(
          'Authorization', 'Bearer ' || current_setting('app.service_role_key', true),
          'Content-Type', 'application/json'
        ),
...
BEGIN
  IF NEW.status = 'resolved' AND OLD.status IS DISTINCT FROM 'resolved' THEN
    ...
    PERFORM
      net.http_post(
        url := current_setting('app.supabase_url', true) || '/functions/v1/temporary-link-generator',
        headers := jsonb_build_object(
          'Authorization', 'Bearer ' || current_setting('app.service_role_key', true),
          'Content-Type', 'application/json'
        ),
