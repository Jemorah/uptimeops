...
// All users sign in here. Redirected to their portal based on role.
// Admin (cumouat@gmail.com) → /hq
// Customer → /customer
// Engineer → /engineer
// ═══════════════════════════════════════════════════════════════
...
export function LoginPage() {
  ...
  const cleanRedirect = redirectTo.startsWith('http')
    ? new URL(redirectTo).hash.replace(/^#/, '') || '/'
    : redirectTo || '';

  // ── After successful auth, navigate to the right place ──
  function redirectBasedOnRole(role: string) {
    if (cleanRedirect && cleanRedirect !== '/login') {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (mode === 'signin') {
      ...
      if (role) {
        ...
        // Component will unmount — no need to setLoading(false)
        return;
      }
    }
