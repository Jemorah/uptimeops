          ...
          <button
            onClick={async () => {
              ...
              const { error } = await supabase.from('human_escalations').insert({
                ...
                reason: 'Customer requested human engineer review',
                escalated_by: user?.id,
              });
              if (error) console.error('Escalation failed:', error);
            }}
            className="w-full flex items-center justify-center gap-2 px-4 py-3 bg-white/5 border border-white/10 text-white/60 text-sm font-bold hover:bg-white/10 transition-colors rounded-xl"
          >
                ...
                {invoices.map(inv => (
                  <div key={inv.id} className="flex items-center justify-between p-3">
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <button
                        onClick={() => {
                          const text = `Invoice: ${inv.id}\nAmount: ${fmtCurrency(inv.amount_cents)}\nStatus: ${inv.status}\nDate: ${fmtDate(inv.created_at)}\n\nGenerated by UptimeOps`;
